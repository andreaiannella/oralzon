  ...
  sections: [
    ...
    },
    {
      ...
      paragraphs: [
        ...
        "Os resultados de pesquisa são ordenados combinando a **correspondência com o termo pesquisado** e alguns parâmetros relativos ao produto. A correspondência mantém-se o fator dominante: os restantes parâmetros determinam a ordem **entre produtos igualmente relevantes**, não para colocar um produto menos relevante à frente de um mais relevante."
      ],
      bullets: [
        "**Correspondência com a pesquisa** — é o parâmetro prevalecente e nenhum outro o pode inverter. A pesquisa compara o termo introduzido com o nome do produto (também traduzido), a marca, o código de artigo e a descrição, com peso decrescente nessa ordem: uma correspondência no nome vale mais do que a mesma palavra presente apenas na descrição",
        "**Filtros e ordenação escolhidos pelo comprador** — quando o comprador ordena por preço, essa escolha prevalece sobre qualquer outro parâmetro, incluindo as posições pagas",
        "**Disponibilidade** — em igualdade de correspondência, um produto disponível precede um esgotado. É o segundo parâmetro por importância, porque um resultado que não se pode comprar não é útil nem ao comprador nem ao vendedor. Os produtos esgotados permanecem visíveis e não são removidos dos resultados",
        "**Vendas realizadas** — em igualdade de correspondência, um produto já adquirido por outros profissionais precede um sem histórico de vendas. O efeito é progressivo mas decrescente: a diferença entre nenhuma venda e as primeiras conta muito mais do que a que existe entre muitas e muitíssimas, pelo que um produto consolidado não ocupa a posição de forma permanente",
        "**Avaliações recebidas** — média das classificações, ponderada pelo seu número: poucas avaliações excelentes pesam menos do que muitas avaliações boas. Só são admitidas avaliações de compradores que tenham efetivamente adquirido esse produto na plataforma",
        "**Produtos publicados recentemente** — os produtos publicados há pouco recebem uma vantagem explícita no posicionamento, que se reduz gradualmente nos primeiros três meses. É uma escolha deliberada: sem ela um marketplace favoreceria de forma estável quem já vende, e um vendedor que entra hoje não teria como começar",
        "**Posicionamento pago** — os vendedores podem adquirir pacotes de visibilidade (produtos em destaque, espaços na página inicial, espaços por categoria, cartões contextuais). Estes conteúdos estão **sempre assinalados como «Patrocinado»**. Nos resultados de pesquisa o patrocínio **soma-se** à pontuação do produto e não a multiplica: pode portanto prevalecer em igualdade de correspondência, mas **não pode colocar um produto pouco relevante acima de um muito relevante**. Quando um espaço pago está disponível mas nenhum vendedor o adquiriu, mostramos um produto não patrocinado com a etiqueta neutra «Em destaque», sem lhe atribuir um patrocínio inexistente",
        "**Histórico de compra e navegação do comprador** — utilizado para sugerir produtos relevantes, com dados recolhidos unicamente nesta plataforma. Não afeta preços nem condições e nunca prevalece sobre as escolhas explícitas do comprador nem sobre os espaços pagos",
        "**Nenhuma preferência por vendedor** — a antiguidade, o volume global de vendas do vendedor, o plano subscrito e a eventual aquisição de outros serviços não influenciam de modo algum o posicionamento dos seus produtos. A Oralzon não vende produtos próprios e não tem por isso posições a favorecer"
      ],
    },
    {
