...
export function VendorFiscale() {
  ...
  const downloadFiscalCSV = () => {
    const headers = ['Ordine', 'Data', 'Cliente', 'P.IVA cliente', 'Prodotto', 'Quantità', 'Prezzo unitario', 'Imponibile', 'Aliquota IVA', 'IVA', 'Reverse charge', 'Da verificare'];
    const csvRows: (string | number)[][] = [];
    fiscalOrders.forEach(o => {
      o.items.forEach(it => {
        csvRows.push([
          o.orderNumber, new Date(o.date).toLocaleDateString('it-IT'), o.customerName, o.customerVat || '',
          it.name, it.quantity, it.unitPrice.toFixed(2), it.net.toFixed(2),
          `${(it.vatRate * 100).toFixed(1)}%`, it.vat.toFixed(2),
          it.reverseCharge ? 'Sì' : 'No', o.taxNeedsReview ? 'Sì' : '',
        ]);
      });
    });
    ...
    <div className="space-y-6">
      ...
      <div className="flex gap-1 bg-gray-100 rounded-lg p-1 w-fit">
        ...
        <button onClick={() => setActiveTab('fatturazione')}
          ...
          <Receipt className="w-4 h-4" /> Dati per fatturazione
        </button>
      </div>
        ...
        <>
          <div className="flex items-start justify-between flex-wrap gap-3">
            <div className="bg-accent border border-oralzon-mint-fresh/30 rounded-xl p-4 text-sm text-oralzon-steel-ink flex gap-3 flex-1 min-w-[280px]">
              ...
              <div>
                ...
                <p>Non è una fattura — è il calcolo già pronto (imponibile, aliquota, IVA, motivo dell'eventuale esenzione) che ti serve per emettere la tua fattura fiscale reale, tua o del tuo commercialista.</p>
              </div>
            </div>
          ...
          {fiscalLoading ? (
            ...
            <div className="bg-white rounded-xl border border-gray-200 text-center py-12">
              ...
              <p className="text-gray-500 font-medium">Nessun ordine pagato ancora</p>
              <p className="text-gray-400 text-sm mt-1">I dati appariranno qui non appena avrai il primo ordine pagato</p>
            </div>
          ) : (
                ...
                <div key={o.orderId} className="bg-white rounded-xl border border-gray-200 overflow-hidden">
                  <button onClick={() => setExpandedOrder(expandedOrder === o.orderId ? null : o.orderId)}
                    ...
                    <div className="flex items-center gap-3 flex-wrap">
                      ...
                      <span className="text-xs text-gray-500">{new Date(o.date).toLocaleDateString('it-IT')}</span>
                      <span className="text-sm text-gray-600">{o.customerName}</span>
                      {o.customerVat && <span className="text-xs text-gray-400 font-mono">P.IVA {o.customerVat}</span>}
                      {o.taxNeedsReview && (
                  ...
                  {expandedOrder === o.orderId && (
                    <div className="border-t border-gray-100 px-4 py-3">
                      ...
                      <table className="w-full text-xs mb-3">
                        <thead>
                          <tr className="text-gray-400 uppercase tracking-wide">
                            <th className="text-left py-1.5">Prodotto</th>
                            <th className="text-right py-1.5">Qtà</th>
                            <th className="text-right py-1.5">Imponibile</th>
                            <th className="text-right py-1.5">Aliquota</th>
                        ...
                        <tbody>
                          {o.items.map((it, i) => (
                            <tr key={i} className="border-t border-gray-50">
                              ...
                              <td className="py-1.5 text-right text-gray-500">{it.reverseCharge ? 'Reverse charge' : `${(it.vatRate * 100).toFixed(0)}%`}</td>
                      ...
                      <div className="flex justify-end gap-6 text-sm border-t border-gray-100 pt-2">
                        <span className="text-gray-500">Imponibile: <strong className="text-gray-800">€{o.netTotal.toFixed(2)}</strong></span>
                        <span className="text-gray-500">IVA: <strong className="text-gray-800">€{o.vatTotal.toFixed(2)}</strong></span>
                        <span className="text-gray-500">Totale: <strong className="text-gray-900">€{o.grossTotal.toFixed(2)}</strong></span>
                      </div>
                    </div>
                  )}
                </div>
