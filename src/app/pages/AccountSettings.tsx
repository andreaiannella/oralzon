    ...
    <div className="max-w-2xl space-y-6">
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6">
        ...
        {[
          ...
          { key: 'promotions', label: t('settings.offersAndPromos'), desc: t('settings.offersDesc') },
        ].map(n => (
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6">
        <div className="flex items-center gap-3 mb-5">
          <Lock className="w-5 h-5 text-green-600" /><h2 className="text-lg font-bold">{t('settings.security')}</h2>
        </div>
        ...
        {!showPasswordForm ? (
          <button onClick={() => setShowPasswordForm(true)} className="w-full text-left px-4 py-3 border border-gray-200 rounded-lg hover:bg-gray-50">
            ...
            <p className="text-xs text-gray-500">{t('settings.updateLoginPassword')}</p>
          </button>
        ) : (
          <form onSubmit={handleChangePassword} className="space-y-3">
            ...
            <div className="relative">
              <input type={showPw ? 'text' : 'password'} placeholder={t('settings.newPasswordPlaceholder')}
            ...
            <input type="password" placeholder={t('settings.confirmPasswordPlaceholder')} value={passwords.confirm}
            ...
            <div className="flex gap-2">
              <button type="submit" disabled={pwLoading} className="flex-1 py-2.5 bg-primary text-white rounded-lg text-sm font-medium flex items-center justify-center gap-2">
                {pwLoading && <Loader2 className="w-4 h-4 animate-spin" />}{t('settings.updatePasswordBtn')}
              </button>
              <button type="button" onClick={() => setShowPasswordForm(false)} className="px-4 py-2.5 border border-gray-300 rounded-lg text-sm">{t('settings.cancel')}</button>
            </div>
          </form>
        )}
      </div>
