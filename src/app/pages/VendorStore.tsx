...
export function VendorStore() {
  ...
  const loadStore = async () => {
    ...
    try {
      ...
      let query = supabase.from('products').select('id, name, price, images, stock')
  ...
  if (!vendor) return (
    <div className="max-w-lg mx-auto px-4 py-20 text-center">
      ...
      <h2 className="text-xl font-bold text-gray-900 mb-2">Store non trovato</h2>
      <Link to="/negozio" className="px-6 py-3 bg-primary text-white rounded-lg inline-block">Torna al Negozio</Link>
    </div>
  );

  const memberSince = new Date(vendor.created_at).toLocaleDateString('it-IT', { month: 'long', year: 'numeric' });
    ...
    <div className="min-h-screen bg-gray-50">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-5">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 flex-wrap">
                ...
                {vendor.verified_badge && (
                  <span className="flex items-center gap-1 text-xs font-semibold text-primary bg-primary/10 px-2 py-0.5 rounded-full">
                    <ShieldCheck className="w-3.5 h-3.5" /> Venditore Verificato
                  </span>
                )}
              </div>
              ...
              <div className="flex items-center gap-4 mt-2 flex-wrap">
                ...
                <span className="text-sm text-gray-400 flex items-center gap-1"><MapPin className="w-3.5 h-3.5" /> Su Oralzon da {memberSince}</span>
              </div>
            </div>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-bold text-gray-900">Prodotti di {vendor.business_name} <span className="text-gray-400 font-normal">({products.length})</span></h2>
