...
interface Address {
  ...
  zip_code: string; city: string; province: string; phone: string; is_default: boolean;
}

interface Props {
  onSelect: (addr: { firstName: string; lastName: string; address: string; zipCode: string; city: string; province: string; phone: string }) => void;
}
...
export function AddressBook({ onSelect }: Props) {
  ...
  const [form, setForm] = useState({ label: '', full_name: '', address: '', zip_code: '', city: '', province: '', phone: '' });
  ...
  const saveAddress = async () => {
    ...
    setShowForm(false); setForm({ label: '', full_name: '', address: '', zip_code: '', city: '', province: '', phone: '' });
    loadAddresses(); setSaving(false);
  };

  const selectAddr = (a: Address) => {
    const [firstName, ...rest] = a.full_name.split(' ');
    onSelect({ firstName, lastName: rest.join(' '), address: a.address, zipCode: a.zip_code, city: a.city, province: a.province, phone: a.phone });
  };
          ...
          {addresses.map(a => (
            <div key={a.id} className="flex-shrink-0 w-48 bg-gray-50 border border-gray-200 rounded-xl p-3 relative group">
              <button onClick={() => selectAddr(a)} className="text-left w-full">
                ...
                <p className="text-[10px] text-gray-500 mt-0.5 line-clamp-2">{a.address}, {a.zip_code} {a.city}</p>
