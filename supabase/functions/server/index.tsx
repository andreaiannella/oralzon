...
// ═══════════════════════════════════════════════════════════════════
// CONVERSAZIONI MASCHERATE cliente↔venditore
// ═══════════════════════════════════════════════════════════════════
// Sostituiscono la mailto: diretta che mostrava pubblicamente l'email reale
// del venditore. Ogni conversazione ha un alias univoco (conv-{id}@dominio)
// usato come mittente/reply-to nelle email — né cliente né venditore vedono
// mai l'indirizzo reale dell'altra parte. IMPORTANTE: l'oscuramento
// dell'indirizzo da solo non basta — nulla impedirebbe di scrivere un
// numero di telefono DENTRO il testo del messaggio. Ogni messaggio, in
// entrambe le direzioni, passa quindi anche da moderateMessageText() prima
// di essere inoltrato.

/**
 * Verifica se un messaggio di testo contiene un tentativo di scambiarsi
 * contatti diretti per aggirare la piattaforma. Fallisce "aperto" (ok:true)
 * in caso di errore tecnico — non blocchiamo mai una conversazione legittima
 * per un problema nostro di infrastruttura, solo per un contenuto rilevato.
 */
async function moderateMessageText(text: string): Promise<{ ok: boolean; reason?: string }> {
  const apiKey = Deno.env.get("ANTHROPIC_API_KEY");
  if (!apiKey) return { ok: true };
  if (!text || text.length > 5000) return { ok: true }; // testo vuoto o assurdamente lungo: non tentiamo la moderazione
  ...
  try {
    const res = await fetch("https://api.anthropic.com/v1/messages", {
      ...
      body: JSON.stringify({
        ...
        messages: [{
          role: "user",
          content: `Questo è un messaggio scambiato tra un cliente e un venditore su Oralzon, marketplace B2B odontoiatrico, tramite il modulo di contatto ufficiale della piattaforma. Verifica se il messaggio contiene un tentativo di scambiarsi contatti diretti per concludere transazioni fuori dalla piattaforma (evitando così la commissione dovuta): un numero di telefono, un indirizzo email, un handle WhatsApp/Telegram, oppure un invito esplicito a "sentirsi altrove" o "pagare direttamente".\n\nMessaggio:\n"""${text}"""\n\nRispondi SOLO con un oggetto JSON valido, nessun testo prima o dopo: {"blocked": true/false, "reason": "breve spiegazione in italiano, o stringa vuota se non trovato nulla"}`,
        }],
      }),
    });
    if (!res.ok) { console.warn("⚠️ Moderazione messaggio — API Anthropic ha risposto", res.status); return { ok: true }; }
    const data = await res.json();
    const text2 = (data.content || []).filter((b: any) => b.type === "text").map((b: any) => b.text).join("");
    const cleaned = text2.replace(/^```json\s*|\s*```$/g, "").trim();
    const parsed = JSON.parse(cleaned);
    if (parsed.blocked) return { ok: false, reason: parsed.reason || "Il messaggio sembra contenere un tentativo di scambio di contatti diretti." };
    return { ok: true };
  } catch (e: any) {
    console.warn("⚠️ Moderazione messaggio fallita, invio consentito per non bloccare una conversazione legittima:", e.message);
    return { ok: true };
  }
}

function inboundDomain(): string {
  return Deno.env.get("INBOUND_MAIL_DOMAIN") || "mail.oralzon.com";
}

// ── CLIENTE: invia il primo messaggio a un venditore tramite conversazione mascherata ──
app.post("/make-server-000b3cfb/vendor/contact", rateLimit(10, 60_000), async (c) => {
  try {
    const auth = await requireAuth(c);
    if (!auth.ok) return c.json({ success: false, error: auth.error }, 401);
    if (!auth.email) return c.json({ success: false, error: "Email utente non disponibile" }, 400);

    const { vendorId, message } = await c.req.json();
    if (!vendorId || !message?.trim()) return c.json({ success: false, error: "Dati mancanti" }, 400);

    const supabase = getServiceClient();
    const { data: vendor } = await supabase.from("vendors").select("id, business_name, contact_email").eq("id", vendorId).maybeSingle();
    if (!vendor?.contact_email) return c.json({ success: false, error: "Questo venditore non è al momento raggiungibile." }, 404);

    const check = await moderateMessageText(message.trim());
    if (!check.ok) return c.json({ success: false, error: `Messaggio non inviato: ${check.reason} Ricorda che gli scambi devono avvenire tramite Oralzon.` }, 400);

    // Una sola conversazione per coppia cliente-venditore (indice unico) —
    // se esiste già, riusiamo quella invece di crearne una nuova ogni volta.
    let { data: conv } = await supabase.from("conversations")
      .select("id").eq("vendor_id", vendorId).eq("customer_id", auth.userId).maybeSingle();
    if (!conv) {
      const { data: newConv, error } = await supabase.from("conversations").insert([{
        vendor_id: vendorId, customer_id: auth.userId,
        vendor_email: vendor.contact_email, customer_email: auth.email,
      }]).select("id").single();
      if (error) throw new Error(error.message);
      conv = newConv;
    }

    await supabase.from("conversation_messages").insert([{ conversation_id: conv!.id, sender_type: "customer", body: message.trim() }]);

    const alias = `conv-${conv!.id}@${inboundDomain()}`;
    const sent = await sendEmail(vendor.contact_email, `Nuovo messaggio da un cliente Oralzon`,
      emailWrapper({
        preheader: "Hai ricevuto un nuovo messaggio da un cliente tramite Oralzon",
        badgeIcon: "message", badgeColor: BRAND_BLUE,
        title: "Nuovo messaggio da un cliente",
        bodyHtml: `
          <p>Ciao <strong>${vendor.business_name}</strong>,</p>
          <p>Un cliente ti ha scritto tramite Oralzon:</p>
          <div style="background:#f9fafb;border-radius:10px;padding:12px 16px;margin:10px 0;">
            <p style="margin:0;font-size:13px;color:#374151;white-space:pre-wrap;">${message.trim()}</p>
          </div>
          <p style="color:#6b7280;font-size:13px;">Rispondi direttamente a questa email: la tua risposta arriverà al cliente senza che nessuno dei due veda l'indirizzo email reale dell'altro.</p>
        `,
      }),
      alias
    );
    if (!sent) return c.json({ success: false, error: "Invio non riuscito, riprova." }, 500);

    return c.json({ success: true });
  } catch (e: any) {
    console.error("❌ vendor/contact:", e);
    return c.json({ success: false, error: e.message }, 500);
  }
});

// ── WEBHOOK: risposte in arrivo via Resend Inbound (email.received) ──
// NOTA SICUREZZA: verifica firma webhook (Svix) non ancora implementata —
// vedi messaggio di risposta per i dettagli. Da aggiungere prima che questo
// canale gestisca volumi reali.
app.post("/make-server-000b3cfb/webhooks/resend-inbound", async (c) => {
  try {
    const body = await c.req.json();
    if (body.type !== "email.received") return c.json({ received: true });

    const emailId = body.data?.email_id;
    const toAddresses: string[] = (body.data?.to || []).map((a: string) => a.toLowerCase());
    const fromAddress: string = (body.data?.from || "").toLowerCase();
    if (!emailId) return c.json({ received: true });

    // Trova l'alias conv-{uuid}@... tra i destinatari — se non c'è, non è
    // una nostra conversazione (es. altre email arrivate sullo stesso dominio inbound).
    let conversationId: string | null = null;
    for (const addr of toAddresses) {
      const match = addr.match(/^conv-([0-9a-f-]{36})@/);
      if (match) { conversationId = match[1]; break; }
    }
    if (!conversationId) return c.json({ received: true });

    const supabase = getServiceClient();
    const { data: conv } = await supabase.from("conversations").select("*").eq("id", conversationId).maybeSingle();
    if (!conv) return c.json({ received: true });

    const isFromVendor = fromAddress.includes(conv.vendor_email.toLowerCase());
    const isFromCustomer = fromAddress.includes(conv.customer_email.toLowerCase());
    if (!isFromVendor && !isFromCustomer) {
      console.warn(`⚠️ webhooks/resend-inbound: mittente non riconosciuto per conversazione ${conversationId}: ${fromAddress}`);
      return c.json({ received: true });
    }

    // Il webhook contiene solo i metadati — il corpo del messaggio va
    // recuperato con una chiamata separata all'API Resend.
    const resendKey = Deno.env.get("RESEND_API_KEY");
    if (!resendKey) return c.json({ received: true });
    const emailRes = await fetch(`https://api.resend.com/emails/receiving/${emailId}`, {
      headers: { Authorization: `Bearer ${resendKey}` },
    });
    if (!emailRes.ok) { console.error("❌ webhooks/resend-inbound: impossibile recuperare il corpo email", emailRes.status); return c.json({ received: true }); }
    const emailData = await emailRes.json();
    const messageText: string = (emailData.text || emailData.html || "").trim();
    if (!messageText) return c.json({ received: true });

    const senderType: "vendor" | "customer" = isFromVendor ? "vendor" : "customer";
    const check = await moderateMessageText(messageText);

    await supabase.from("conversation_messages").insert([{
      conversation_id: conversationId, sender_type: senderType, body: messageText,
      blocked: !check.ok, block_reason: check.reason || null,
    }]);

    const senderRealEmail = isFromVendor ? conv.vendor_email : conv.customer_email;
    const recipientRealEmail = isFromVendor ? conv.customer_email : conv.vendor_email;
    const alias = `conv-${conversationId}@${inboundDomain()}`;

    if (!check.ok) {
      // Messaggio bloccato: non lo inoltriamo, avvisiamo solo chi l'ha scritto.
      await sendEmail(senderRealEmail, "Messaggio non recapitato — Oralzon",
        emailWrapper({
          badgeIcon: "undo", badgeColor: "#dc2626",
          title: "Messaggio non recapitato",
          bodyHtml: `
            <p>Il tuo ultimo messaggio non è stato inoltrato: ${check.reason}</p>
            <p style="color:#6b7280;font-size:13px;">Per tutelare acquirenti e venditori, le comunicazioni su Oralzon non possono contenere contatti diretti (telefono, email, WhatsApp). Riscrivi il messaggio senza queste informazioni.</p>
          `,
        })
      );
      return c.json({ received: true });
    ...
    await sendEmail(recipientRealEmail, `Nuovo messaggio — Oralzon`,
      emailWrapper({
        preheader: "Hai ricevuto un nuovo messaggio tramite Oralzon",
        badgeIcon: "message", badgeColor: BRAND_BLUE,
        title: "Nuovo messaggio",
        bodyHtml: `
          <div style="background:#f9fafb;border-radius:10px;padding:12px 16px;margin:10px 0;">
            <p style="margin:0;font-size:13px;color:#374151;white-space:pre-wrap;">${messageText}</p>
          </div>
          <p style="color:#6b7280;font-size:13px;">Rispondi direttamente a questa email per continuare la conversazione in modo sicuro tramite Oralzon.</p>
        `,
      }),
      alias
    );
    ...
    console.error("❌ webhooks/resend-inbound:", e);
    return c.json({ received: true }); // 200 comunque: Resend non deve ritentare all'infinito su un nostro errore interno
  }
});
